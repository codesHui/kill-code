...
@Controller(value="bjsAdminPolicyController")
@RequestMapping(value = "/bjsAdmin/policy")
...
//			complaintsServiceImpl.findDisposeComplaints(productInfo);
		...
		return "policy_bjs/policy_summary";
	}
...
//			complaintsServiceImpl.findDisposeComplaints(productInfo);
	...
	public String query(){
		return "policy_bjs/policy_query";
	}
	/**
	 * 点击详情
	 * @param request
...
//			complaintsServiceImpl.findDisposeComplaints(productInfo);
		...
		return "policy_bjs/policy_"+riskType;
	}
	...
	public void upload(@ModelAttribute(value="policyInformation")PolicyInformation policyInformation,HttpServletRequest request,HttpServletResponse response){
		...
		if(!StringUtils.isBlank(riskType)){
			...
			if(reportPolicyList.size()>0){
				for(ReportPolicy i:reportPolicyList){
					...
					}else if("05OP".equals(i.getRiskType())){
						reportPolicy05OPList.add(i);
					}else if("05PI".equals(i.getRiskType())){
						reportPolicy05PIList.add(i);
					}else if("09IE".equals(i.getRiskType())){
						reportPolicy09IEList.add(i);
					}else if("08OV".equals(i.getRiskType())){
